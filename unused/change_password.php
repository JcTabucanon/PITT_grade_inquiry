        ...
        <section class="main-container">
            ...
            <div class="right">
                <form method="post" enctype="multipart/form-data">
                    <div class="imgcenter">
                        <img id="preview" src="registrar/<?php echo $row['PHOTO']; ?>" style="width:200px; border-radius:50%; align-items:center; object-fit:cover; height:200px; border:3px solid goldenrod">
                    </div>
                    <fieldset>
                    ...
                    <div class="control-group">
                        <div class="controls">
                            <button name="update" type="submit" class="btn btn-info"><i class="icon-signin icon-large"></i>&nbsp;Update</button>
                        </div>
                        ...
                        if (isset($_POST['update'])) {
                            ...
                            if (!empty($np) && $np != $rp) {
                        ?>
                                <div id="alert-container" class="alert alert-danger">Password Don't Match</div>
                                <?php
                            } else {
                                if (!empty($np)) {
                                    mysqli_query($connection, "UPDATE USERS SET PASSWORD = '$np', photo = '$location' WHERE SCHOOL_ID = '$CURRENT_USERID' ") or die(mysqli_error($connection));
                                ?>
                                    <div id="alert-container" class="alert alert-success">Password Changed</div>
                                    <?php
                                } else {
                                    if (!empty($location)) {
                                        mysqli_query($connection, "UPDATE USERS SET PHOTO = '$location'  ") or die(mysqli_error($connection));
                                    ?>
                                        <div id="alert-container" class="alert alert-success">Image Uploaded</div>
                                        <?php
                                    } else {
                                        if (!empty($np) && !empty($location)) {
                                            mysqli_query($connection, "UPDATE USERS SET PASSWORD = '$np', photo = '$location' WHERE SCHOOL_ID = '$CURRENT_USERID' ") or die(mysqli_error($connection));
                                        ?>
                                            <div id="alert-container" class="alert alert-success">Password and Image Changed</div>
                                        <?php
                                        } else {
                                        ?>
                                            <div id="alert-container" class="alert alert-warning">No changes were made</div>
                        <?php
                                        }
                                    }
                                }
