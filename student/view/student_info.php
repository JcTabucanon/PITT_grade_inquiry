    ...
    <section class="main-container">
      ...
      <div class="right">
        <form method="post" enctype="multipart/form-data">
          <div class="imgcenter">
            <img id="preview" src="../../student/<?php echo $row['PHOTO']; ?>" style="width:200px; border-radius:50%; align-items:center; object-fit:cover; height:200px; border:3px solid goldenrod">
          </div>
          <fieldset>
            ...
            <div class="control-group">
              ...
              <div class="controls">
                <input type="file" name="image" id="image" class="font" onchange="previewImage();">
              </div>
            </div>
          </fieldset>
          <div class="control-group">
            <div class="controls">
              <button name="update" type="submit" class="btn bg-gray-600"><i class="icon-signin icon-large"></i>&nbsp;Update</button>
            </div>
          </div>
        </form>
...
$CURRENT_USERID = $_SESSION['CURRENT_USERID'];
        ...
        if (isset($_POST['update'])) {
          ...
          if (!empty($_FILES['image']['tmp_name'])) {
            ...
            move_uploaded_file($_FILES["image"]["tmp_name"], "../../student/upload/" . $_FILES["image"]["name"]);
            $location = "upload/" . $_FILES["image"]["name"];
          }

          if (!empty($np) && $np != $rp) {
            echo '<div id="alert-container" class="alert alert-danger">Password Don\'t Match</div>';
          } else {
            if (!empty($np)) {
              mysqli_query($conn, "UPDATE users SET PASSWORD = '$np', PHOTO = '$location' WHERE USER_ID = '$CURRENT_USERID' ") or die(mysqli_error($conn));
              echo '<div id="alert-container" class="alert alert-success">Password Changed</div>';
            } else {
              if (!empty($location)) {
                mysqli_query($conn, "UPDATE users SET PHOTO = '$location' WHERE USER_ID = '$CURRENT_USERID' ") or die(mysqli_error($conn));
                echo '<div id="alert-container" class="alert alert-success">Image Uploaded</div>';
              } else {
                if (!empty($np) && !empty($location)) {
                  mysqli_query($conn, "UPDATE users SET PASSWORD = '$np', PHOTO = '$location' WHERE USER_ID = '$CURRENT_USERID' ") or die(mysqli_error($conn));
                  echo '<div id="alert-container" class="alert alert-success">Password and Image Changed</div>';
                } else {
                  echo '<div id="alert-container" class="alert alert-warning">No changes were made</div>';
                }
              }
            }
          }
        ...
        <script>
          $(document).ready(function() {
            $("#alert-container").fadeTo(3000, 500).slideUp(500, function() {
    ...
    <footer class="pt-8 pb-6">
      <p class="text-sm text-center text-gray-400">&#169; 2023 Program by<b>Wendel Luche & Jessie Cuna</b>. All rights reserved.</p>
    </footer>
  </main>
</body>
</html>
